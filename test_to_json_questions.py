...
def process_and_save_from_local(images_folder, qa_pairs, output_folder, subset_name):
    ...
    for image_name, question in qa_pairs:
        ...
        print(f"Added entry for image {image_name} with question: {question}")
...
def load_qa_pairs(qa_file):
    ...
    with open(qa_file, 'r') as f:
        for line in f:
            ...
            if len(parts) == 2:  # Expecting only image_name and question
                image_name, question = parts
                qa_pairs.append((image_name, question))
            else:
